import type { QrErrorCorrectionLevel, QrSegmentMode } from './types';
...
export interface QrVersionEccInfo {
  version: number;
...
const EC_LEVELS: ReadonlyArray<QrErrorCorrectionLevel> = ['L', 'M', 'Q', 'H'];
...
export const FORMAT_EC_BITS: Record<QrErrorCorrectionLevel, number> = {
  L: 1,
  M: 0,
  Q: 3,
  H: 2,
...
const ALIGNMENT_PATTERN_LOCATIONS: ReadonlyArray<ReadonlyArray<number>> = [
  [],
  [],
  [6, 18],
  [6, 22],
...
const VERSION_INFO_BY_KEY = new Map<string, QrVersionEccInfo>();

for (const line of RAW_RS_BLOCKS.split('\n')) {
  const [versionLevel, totalDataStr, ecCodewordsPerBlockStr, group1CountStr, group1DataStr, group2CountStr, group2DataStr] =
    line.trim().split(/\s+/);
  const [versionStr, ecLevel] = versionLevel.split('-');
  const version = Number(versionStr);
  const totalData = Number(totalDataStr);
  const ecCodewordsPerBlock = Number(ecCodewordsPerBlockStr);
  const groups: QrBlockGroup[] = [
    { count: Number(group1CountStr), dataCodewords: Number(group1DataStr) },
  ];
  if (group2CountStr !== undefined && group2DataStr !== undefined) {
    groups.push({
      count: Number(group2CountStr),
      dataCodewords: Number(group2DataStr),
    });
  }
  const totalBlocks = groups.reduce((sum, group) => sum + group.count, 0);
  VERSION_INFO_BY_KEY.set(`${version}-${ecLevel}`, {
    version,
    errorCorrection: ecLevel as QrErrorCorrectionLevel,
    dataCodewords: totalData,
    ecCodewordsPerBlock,
    groups,
    totalCodewords: totalData + totalBlocks * ecCodewordsPerBlock,
  });
}

export function getVersionInfo(version: number, errorCorrection: QrErrorCorrectionLevel): QrVersionEccInfo {
  const info = VERSION_INFO_BY_KEY.get(`${version}-${errorCorrection}`);
  if (!info) {
    throw new Error(`Unsupported QR version/error correction combination: ${version}-${errorCorrection}`);
  }
  return info;
}

export function getAlignmentPatternPositions(version: number): ReadonlyArray<number> {
  if (version < 1 || version > 40) {
    throw new Error(`QR version must be between 1 and 40. Received ${version}.`);
  }
  return ALIGNMENT_PATTERN_LOCATIONS[version];
}

export function getCharacterCountBits(mode: QrSegmentMode, version: number): number {
  if (version <= 9) {
...
export function getAllErrorCorrectionLevels(): ReadonlyArray<QrErrorCorrectionLevel> {
  return EC_LEVELS;
}
